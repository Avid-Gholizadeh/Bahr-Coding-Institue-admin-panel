...
const Wizard = forwardRef((props, ref) => {
    ...
    useEffect(() => {
        ...
        ref.current.addEventListener('shown.bs-stepper', async function (event) {
            if (news) {
            ...
            // console.log('from : ', event.detail.from + 1)
            const validSubmit = await submitForm(event.detail.from + 1)

            // console.log(event.detail)

            if (!validSubmit) {
                // console.log('go to :', event.detail.indexStep + 1)
                stepper.to(event.detail.from + 1)
            } else {
                // console.log(event.detail.from)
                setActiveIndex(prevS => {
                    return event.detail.indexStep
                })
                // console.log('before : ', event.detail.indexStep + 1)
                stepper.to(event.detail.indexStep + 1)
            }
        })
